...
class PaymentEspece(models.Model):
    _name = 'softy.pespece'
    _description = 'Payment Espece'
    _rec_name = 'ref'
    ...
    ref = fields.Char(string='Réf. Paiement Espèce', required=True)
    date = fields.Date(string='Date', required=True, default=fields.Date.context_today)
    
    # CHANGED: Convert from related field to regular stored field
    representant = fields.Char(
        string='Représentant',
        store=True,
        readonly=True
    )
    ...
    def create(self, vals):
        """Override create to populate fields from bulletin when Payment Espece is created"""
        if 'bulletin_id' in vals and vals['bulletin_id']:
            ...
            if bulletin.exists():
                ...
                vals.update({
                    ...
                    'representant': bulletin.departement_id.representant if bulletin.departement_id else False,
    ...
    def generate_espece_from_bulletins(self):
        """Generate Paiement Espèce records from bulletins where employee payment mode is 'esp' only"""
        # Filter bulletins to only include those where employee mode_payment is 'esp' (Espèce)
        bulletins = self.env['softy.bulletin'].search([
            ('employe_id.mode_payment', '=', 'esp')
        ])
        ...
        for bulletin in bulletins:
            # Check if Paiement Espèce already exists for this bulletin
            existing_espece = self.search([('bulletin_id', '=', bulletin.id)])
            if not existing_espece:
                # Generate reference automatically
                ref = f"ESP-{bulletin.employe_id.matricule or 'UNKNOWN'}-{bulletin.mois}/{bulletin.annee}"
        ...
        espece_bulletins = len(bulletins)
        
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': 'Génération Paiement Espèce',
                'message': f'{created_count} enregistrements Paiement Espèce créés avec succès.\n'
                          f'{skipped_count} déjà existants.\n'
                          f'Traités: {espece_bulletins} bulletins avec mode "Espèce" sur {total_bulletins} bulletins totaux.',
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    def action_print_receipt(self):
        """Action to print receipt report - collective if multiple records, individual if single"""
        if len(self) > 1:
            # For multiple records, use collective template
            return self.env.ref('softy_hr.report_espece_receipt_collective_document').report_action(self)
        else:
            # For single record, use individual template
            return self.env.ref('softy_hr.report_espece_receipt_document').report_action(self)

    def action_print_collective_receipt(self):
        """Action to print collective receipt report for selected records"""
        return self.env.ref('softy_hr.report_espece_receipt_collective_document').report_action(self)

    @api.model
    def get_month_name_french(self, month_number):
        ...
        return french_months.get(str(month_number), str(month_number))